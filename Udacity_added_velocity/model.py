...
from models import get_velocity_branched_model
...
def load_data(args):
    ...
    for filename in glob.iglob(path.join(args.data_dir, '**\\*.csv'), recursive=True):
        ...
        if X_train is None:
            ...
            X_train_tmp, X_valid_tmp, Y_train_tmp, Y_valid_tmp = train_test_split(X, y, test_size=args.test_size,
                                                                                  random_state=0)
    ...
    print('Data size: Total items - {}        Train - {}        Validation - {}'.format(len(X_train) + len(X_valid),
                                                                                        len(X_train), len(X_valid)))
    return X_train, X_valid, Y_train, Y_valid


def train_model(save_dir, model, augment_prob, batch_size, small_angle_keep_prob, translate_multiplier, args, X_train,
    ...
    model.fit_generator(
        batch_generator(args.data_dir, X_train, y_train, batch_size, True, augment_prob, small_angle_keep_prob,
...
def train(normalize, pooling_type, add_dense, activation_conv, activation_dense, augment_prob, batch_size,
          small_angle_keep_prob, translate_multiplier, data, args):
    save_dir = 'with_speed_normalize={}_pooling={}_addDence={}_activationConv={}_activationDence={}_augmentProb={}_batchSize={}_smallAngleKeepProb={}_translateMult={}'.format(
        ...
        small_angle_keep_prob, translate_multiplier)
    model = build_parameterized_net(normalize=normalize, pooling_type=pooling_type, add_dense=add_dense,
                                    activation_conv=activation_conv, activation_dense=activation_dense)
    train_model(save_dir, model, augment_prob, batch_size, small_angle_keep_prob, translate_multiplier, args, *data)


def train_built_model(normalize, pooling_type, add_dense, activation_conv, activation_dense, augment_prob, batch_size,
                      small_angle_keep_prob, translate_multiplier, data, args):
    save_dir = 'with_speed_normalize={}_pooling={}_addDence={}_activationConv={}_activationDence={}_augmentProb={}_batchSize={}_smallAngleKeepProb={}_translateMult={}'.format(
        normalize, pooling_type, add_dense, activation_conv, activation_dense, augment_prob, batch_size,
        small_angle_keep_prob, translate_multiplier)
...
if __name__ == '__main__':
    ...
    data = load_data(args)
    counter = 0
    for aug_prob, batch_size, angle_keep_prob, translate_mult, norm_value, pool_type, add_dense, act_conv, act_dense in itertools.product(
            augment_prob_value, batch_size_values, small_angle_keep_prob_values, translate_mult_values,
            normalize_values, pooling_values, add_dense_values, activation_values_conv, activation_values_dense):
        train_built_model(normalize=norm_value,
